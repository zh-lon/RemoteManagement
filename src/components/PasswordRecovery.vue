    ...
    <el-card header="密码恢复工具">
      <div class="recovery-section">
        ...
        <div v-if="encryptionStatus" class="status-result">
          ...
          <div v-if="encryptionStatus.details" class="status-details">
            <h4>详细信息</h4>
            <div v-for="(value, key) in encryptionStatus.details" :key="key" class="detail-item">
      ...
      <div class="recovery-section">
        ...
        <div class="recovery-option">
          ...
          <el-button @click="regenerateKey" type="warning" :loading="regenerating">
      ...
      <div v-if="showFixer" class="recovery-section">
        ...
        <div v-else>
          <div v-for="connection in affectedConnections" :key="connection.id" class="connection-item">
            ...
            <div class="password-fix">
              <el-input
                ...
                size="small"
              />
              <el-button @click="fixPassword(connection)" type="success" size="small">
                修复
              </el-button>
            </div>
          </div>
          ...
          <div class="fix-actions">
            <el-button @click="fixAllPasswords" type="primary" :loading="fixingAll">
      ...
      <div v-if="recoveryResults.length > 0" class="recovery-section">
        <h3>操作结果</h3>
        <div v-for="result in recoveryResults" :key="result.id" class="recovery-result">
...
const affectedConnections = ref<Array<ConnectionItem & { newPassword: string }>>([])
const recoveryResults = ref<Array<{
  id: number
  success: boolean
  message: string
  time: string
...
const checkEncryptionStatus = async () => {
  try {
    ...
    const fingerprintMatch = encryptionService.verifyKeyFingerprint()
  ...
  const traverse = (items: Array<ConnectionGroup | ConnectionItem>) => {
    for (const item of items) {
      if (isConnectionItem(item)) {
        // 检查密码是否为空或看起来像加密数据
        if (!item.password || (item.password.length > 20 && /^[A-Za-z0-9+/]*={0,2}$/.test(item.password))) {
          count++
        }
      } else {
...
const regenerateKey = async () => {
  try {
    ...
    const connectionsResult = await storageService.loadConnections()
    if (connectionsResult.success && connectionsResult.data) {
      const clearedGroups = clearPasswordsFromGroups(connectionsResult.data.groups)
...
const clearPasswordsFromGroups = (groups: ConnectionGroup[]): ConnectionGroup[] => {
  ...
  try {
    ...
    if (connectionsResult.success && connectionsResult.data) {
      ...
      if (affectedConnections.value.length === 0) {
        ...
        ElMessage.success(`发现 ${affectedConnections.value.length} 个需要修复的连接`)
      }
    }
  } catch (error) {
...
const fixAllPasswords = async () => {
  try {
    ...
    if (connectionsResult.success && connectionsResult.data) {
      ...
      addResult(true, `已修复 ${affectedConnections.value.length} 个连接的密码`)
...
const updatePasswordsInGroups = (groups: ConnectionGroup[]) => {
  const traverse = (items: Array<ConnectionGroup | ConnectionItem>) => {
    for (const item of items) {
      if (isConnectionItem(item)) {
        const affected = affectedConnections.value.find(conn => conn.id === item.id)
